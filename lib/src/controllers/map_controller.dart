...
class MapController extends ControllerMVC {
  ...
  void listenForNearRestaurants(Address myLocation, Address areaLocation) async {
    ...
    stream.listen((Restaurant _restaurant) {
      ...
      print("Done");
      cameraPosition = CameraPosition(
        target: LatLng(double.parse(topRestaurants[0].latitude), double.parse(topRestaurants[0].longitude)),
        zoom: 8,
      );
    });
  }
  ...
  void getCurrentLocation() async {
    try {
      ...
      // if (!currentAddress.isUnknown()) {
      Helper.getMyPositionMarker(double.parse(currentRestaurant.latitude), double.parse(currentRestaurant.longitude),currentRestaurant.name).then((marker) {
        setState(() {
          allMarkers.add(marker);
        });
      });
  ...
  void getRestaurantLocation() async {
    try {
      ...
      Helper.getMyPositionMarker(currentAddress.latitude, currentAddress.longitude,currentRestaurant.name).then((marker) {
        setState(() {
          allMarkers.add(marker);
        });
      });
